...
def calculate_contrast_equal_weight_signal(
    ...
    """Calculate contrast-style pair signals, raw equal-weight signal, and factor value."""
    if lookback <= 0:
    ...
    output["factor_value"] = raw_signal.rolling(smoothing_window, min_periods=1).mean()
...
def main() -> int:
    ...
    print(f"Latest ({output.index[-1].date()}): factor_value={latest['factor_value']:.4f}")
    return 0
