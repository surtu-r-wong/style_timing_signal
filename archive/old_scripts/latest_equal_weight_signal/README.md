...
3. 计算 20 日滚动复合相对收益。
4. 用 40 日窗口做 z-score 标准化，40 日窗口满后才开始产生非零信号。
5. 用 `tanh(z / 2)` 压缩到约 `[-1, 1]`。
6. 所有组等权平均，得到 `raw_signal_20`，同时输出为 `factor_value_raw`，表示当天未平滑值。
7. 对 `factor_value_raw` 做 5 日滚动平均，得到平滑后的 `factor_value`。
...
```bash
python latest_equal_weight_signal/generate_equal_weight_signal_contrast.py \
  --input data.csv \
  --config latest_equal_weight_signal/style_factor_groups.csv \
...
- `date`: 日期。
- `pair_01_factor_20`, `pair_02_factor_20`, ...: 每组单独的信号。
- `raw_signal_20`: 所有组的等权平均原始信号。
- `factor_value`: 5 日平滑后的最终连续择时信号。

示例：

```csv
date,pair_01_factor_20,pair_02_factor_20,raw_signal_20,factor_value
2024-01-01,0.0000,0.0000,0.0000,0.0000
2024-01-02,0.0000,0.0000,0.0000,0.0000
```
