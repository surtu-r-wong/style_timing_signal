...
from backtest.metrics import ann_return, sharpe, turnover, max_drawdown  # noqa: E402
...
def carry_income(position: pd.Series, carry: pd.Series, start, end) -> float:
    """Annual carry contribution, same effective position as the daily engine.

    Shift before slicing so a held position across a reporting boundary is kept.
    """
    effective = position.shift(1).fillna(0.0).loc[start:end]
    return float((effective * carry.reindex(effective.index).fillna(0.0)).mean())
...
def main() -> int:
    ...
    y = pd.DataFrame({"s": r_s, "s0": r_s0, "pos": ps.shift(1).fillna(0.0).reindex(r_s.index), "c": carry.reindex(r_s.index), "u": und.reindex(r_s.index)}).loc[WINDOW[0]:WINDOW[1]]
    ...
    def income(cs: pd.Series) -> float:
        return round(carry_income(sg, cs, *WINDOW), 4)
    out["carrying_mode_symmetric"] = {"A_natural_50_50": income(pd.concat([c500, c1000], axis=1).fillna(0.0).mean(axis=1)), "B_all_IM": income(c1000.reindex(idx).fillna(c500.reindex(idx))), "C_all_IC": income(c500),
