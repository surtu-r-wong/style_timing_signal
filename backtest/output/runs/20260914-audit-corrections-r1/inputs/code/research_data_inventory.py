...
def inventory():
    ...
    try:
        ...
        with c.cursor() as q:
            ...
            q.execute("SELECT table_schema,table_name,column_name FROM information_schema.columns WHERE (table_schema='stock_selector' AND table_name IN ('index_money_flow','etf_constituent','fund_meta','fund_nav','fund_nav_daily','fund_holder_structure','fund_period_trading')) OR (table_schema='public' AND table_name ILIKE '%member%') ORDER BY 1,2,ordinal_position")
            ...
            codes=('510500.SH','510500.OF','512100.SH','512100.OF','159845.SZ','159845.OF')
            q.execute("SELECT fund_code,name,fund_type FROM stock_selector.fund_meta WHERE fund_code=ANY(%s)",(list(codes),))
            out['illustrative_etf_meta']=q.fetchall()
            q.execute("SELECT fund_code,min(trade_date),max(trade_date),count(*),count(net_asset) FROM stock_selector.fund_nav WHERE fund_code=ANY(%s) GROUP BY 1",(list(codes),))
            out['illustrative_etf_nav_assets']=q.fetchall()
            q.execute("SELECT fund_code,min(trade_date),max(trade_date),count(*) FROM stock_selector.fund_nav_daily WHERE fund_code=ANY(%s) GROUP BY 1",(list(codes),))
            out['illustrative_etf_daily_nav']=q.fetchall()
    ...
    out['interpretation']={
        ...
        'fund_asset_caveat':'fund_nav.net_asset and fund_holder_structure.share_net_asset exist; the three illustrative ETF codes have no metadata/NAV rows. This does not assert all fund-size history is absent.',
