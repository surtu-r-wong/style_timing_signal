"""创新高参与度面·入场券探针（预登记 docs/plans/2026-09-03-new-high-breadth-axis-prereg.md，冻结后才可 --run）。
两族 N1/N2 × GRID_LEVEL × k = 32 变体；默认两半窗 2014-2019/2020-2026；关 1~3 原样 + 关 0（候选 ⓪）。
CLI: python3 -m backtest.new_high_axis_probe --run [--n-perm 1000]
"""
...
from backtest.new_high_breadth import build_series  # noqa: E402
...
# 关 0 历史口径为 max-T；保留当时的阈值与计算。
# 2026-09-14：撤回“此后默认 min-P”的建议。池内排序不验证真实误报率；
# max-T/min-P 均须先校准底层重抽样假设。新研究遵循 PREREG_TEMPLATE，
# 已登记输出不因本注释更正而重算或翻转；详见统计依赖审计报告。
...
def main(argv=None) -> int:
    ...
    if not a.run:
        print(series[["N1", "N2"]].describe().round(4).to_string()); print("nan:", series[["N1", "N2"]].isna().sum().to_dict()); return 0
    ...
    panel.to_csv(OUT_DIR / "new_high_axis_probe.csv", index=False); verdicts.to_csv(OUT_DIR / "new_high_axis_probe_verdicts.csv", index=False)
    (OUT_DIR / "new_high_axis_selection.json").write_text(json.dumps(meta, ensure_ascii=False, indent=1))
