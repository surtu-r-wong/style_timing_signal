"""期指基差期限结构面·入场券探针（预登记 docs/plans/2026-09-03-basis-term-structure-axis-prereg.md，冻结后才可 --run）。
三族 T1/T2/T3 × GRID_LEVEL × k = 48 变体；默认两半窗 2014-2019/2020-2026；关 1~3 原样 + 关 0（候选 ⓪）。
CLI: python3 -m backtest.basis_term_probe --run [--n-perm 1000]
"""
...
from backtest.basis_term import build_series  # noqa: E402
...
def main(argv=None) -> int:
    ...
    if not a.run:
        print(series[["T1", "T2", "T3"]].describe().round(4).to_string()); print("nan:", series[["T1", "T2", "T3"]].isna().sum().to_dict()); return 0
    ...
    panel.to_csv(OUT_DIR / "basis_term_probe.csv", index=False); verdicts.to_csv(OUT_DIR / "basis_term_probe_verdicts.csv", index=False)
    (OUT_DIR / "basis_term_selection.json").write_text(json.dumps(meta, ensure_ascii=False, indent=1))
