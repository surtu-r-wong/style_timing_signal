...
- T 收盘出信号 → **T+1 那一行记收益**：`pos_eff = position.shift(1)`。标的收益是
  收盘对收盘（`close.pct_change()`，第 t 行覆盖 t−1 收盘 → t 收盘），因此本引擎
  **经济上等价于「按 T 收盘价成交」**，而不是设计稿 §3.2 写的「T+1 收盘成交」
  （那需要 `shift(2)`）。该口径差与其敏感性见
