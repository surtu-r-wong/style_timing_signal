...
PY
  ...
  echo "  3. SUSPECT（topup 模式）= 先判是否上游合法回溯修订，再决定是否置 SKIP_TOPUP"
  echo "     OFFICE_SUSPECT（办公室模式，失败步骤 inputs_check(OFFICE_SUSPECT)）= 办公室写入的当日输入没过"
  echo "     同族共动性哨兵（对内价差 ≥8pp / 序列冻结），"
  echo "     信号未重算。看运行日志 OFFICE_SUSPECT 行：数据错了通知 data_manager 办公室核对、改正后重跑本链路；"
  echo "     确属真实行情 → 核实后放行重跑：STYLE_SIGNALS_INPUTS_ARGS=\"--accept-sentinel <信号日>\""
  echo "     ${SCRIPT_DIR}/run_daily_signals.sh（只放行那一天；不处置的话次日运行只判次日、会自动恢复）。"
  echo "     标志文件 SKIP_TOPUP 本来就在，别动它"
