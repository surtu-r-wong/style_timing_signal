...
> **⚠️ 勘误（2026-08-13，执行价口径审计 Batch 12）**：上一条 PIT 论证的**前提是错的**。
> `backtest/engine.py` 的 `pos_eff = position.shift(1)` 配收盘对收盘的标的收益，
> 经济上等价于**按 T 收盘价成交**，而非「T+1 收盘成交」。
> 因此真正的 PIT 要求是「**广度数据在 T 日收盘那一刻已可得**」，
> 比原文的「T 日 EOD 当晚可得」**严格得多**——`stock_daily_price_qfq` 的 T 日收盘价
> 不可能在 T 收盘瞬间全市场落库。**该 PIT 安全性论证需按新前提重述**；
> 本勘误只登记，不重跑广度探针、不改其既有结论。
> 口径差的正式入档与敏感性读数见 `docs/plans/2026-08-13-exec-price-audit.md` §1.2。

> **✅ 重述（2026-08-18，技术债 P2-2 清账）**——按 T 收盘成交的正确前提重写 PIT 论证：
>
> **(1) 正确的 PIT 要求。** 引擎 `pos_eff = position.shift(1)` 配收盘对收盘收益，
> 意味着由 T 日信号形成的仓位必须**在 T 日收盘那一刻已持有**。若广度由 T 日全市场
> 收盘价算出，则「用 T 收盘算信号」与「按 T 收盘成交」发生在同一瞬间 ——
> **严格口径下不可实现**（`stock_daily_price_qfq` 的 T 日收盘价在 T 收盘瞬间不可能
> 全市场落库）。可近似实现的形态是**收盘前快照**（如 14:5x 全市场价）代替收盘价，
> 残差为尾盘几分钟的价格变动。原文「T 日 EOD 当晚可得 → PIT 安全」及其括注
> 「`available_at` 滞后仅威胁 T+1 开盘成交口径」**两句均作废**。
>
> **(2) 这是否动摇 phase4 的 STOP 结论？不动摇，且有量化依据。**
> 本项目对"这一根 bar"的经济含量做过直接测量（执行价审计路线 B，
> `2026-08-13-exec-price-audit.md` §3.1）：把仓位再多等一天（`shift=2` vs `shift=1`），
> full 窗 Sharpe 差 **+0.018 ~ +0.046（九格全为正）**、95% CI 全部跨 0、**p 0.709~0.877**。
> 即**这一根 bar 的信息价值在统计上与 0 不可分**，方向上甚至是"多等一天略好"。
> 而 phase4 广度族的裁决是 **168 组无一组三窗全正、IC≈0（p .73–.94）**。
> 把一个"与 0 不可分"的口径修正加到"本就没有信号"的读数上，不构成翻盘路径。
>
> **(3) 论证的逻辑形式（避免误用）。** 被测广度信号相对可实现版本享有的是
> **弱信息优势**，而"占了便宜仍然什么都没测到"对 STOP 是 **a fortiori** 的支持 ——
> 但这**不是恒等式**：滞后版本原则上可以更好（(2) 中 shift2 九格全正就是实例）。
> 正因如此这里不靠"少信息必更差"这种伪定理，而是靠 (2) 的**实测幅度**：
> 效应小到无法把 IC≈0 变成信号。
>
> **(4) 若本轴将来重开，规范是什么。** 必须显式声明广度腿的可得性口径 ——
> 要么 `breadth.shift(1)`（把广度滞后一根 bar，与 T 收盘成交严格相容），
> 要么显式建模收盘前快照并交代残差。**重开时不得引用本节 §1.1 原论证**（已作废），
> 只能引用本重述块。本次重述**不重跑探针、不改既有结论**（与勘误块一致）。
