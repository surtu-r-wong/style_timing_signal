...
## 5. 冻结后实现更正（2026-09-03，跑前披露，不改闸门与网格）

首次构建发现两处实现与 §2 本意不符，探针已停、未出任何裁决数：
1. `close_hfq` 在 2025 年仅约 90% 覆盖、2026-06-02 后全空 → 改用 `close × adj_factor` 自算后复权（与 close_hfq 逐日相等、比值恒 1.0，复权因子全表无空）；数据尾延到表尾。
2. 「250 日窗须满」原实现为窗内无任何缺行，而表里停牌日无行 → 2015～2016 千股停牌期误剔约三分之二样本（n_market 中位仅 650～740）。
   更正为：停牌日前向填充只用于滚动最高，新高只在有实际收盘的日子判定，分母资格 = 累计实际观测 ≥ 250 日且上市满 15 个月且未退市。
