...
preflight 只读取形成日及以前可知的前五类截面字段；后续个股收益、目标指数收益和 carry 只有在 preflight 通过后才能读取。

有两个截面集合：

1. **size universe**：形成日仍在样本中的沪深 A 股（不含北交所与港股通——2026-07-24 确认口径，代码按 `.BJ`/`.HK` 后缀将其整体排除于样本池外，不进入 universe 计数与排除原因统计），具备有效总市值和行业标签。m、m_perp 与目标市值坐标在该集合上计算。
   **形成日停牌处理（2026-07-24 确认，"带旧价"口径）**：形成日无成交行的股票，若当日有停牌证据（`stock_selector.stock_suspension`，源 = Wind wset `tradesuspend` 日频记录）且存在既往成交收盘价，则按最后成交 close 计总市值留在 size universe——与中证指数"停牌成分按停牌前价格保留"的方法论一致；此类填充在 coverage_audit 以 `close_carry_forward`/`SUSPENDED_CARRY_FORWARD` 的 REPORT_ONLY 行按月计数。无停牌证据或无既往成交价的缺收盘仍记 `DATA_MISSING_CLOSE` 并按输入合同 DATA_BLOCKED——证据缺席不得带旧价（fail-closed）。
...
**已接受的现实预期（2026-07-14）**：当前真实首披日尚未回填、SalG 在 2025Q2 后冻结、futures carry 停在 2026-04-29。首轮实现不负责修复这三项上游数据，因此预期交付是 approximate-PIT 的 statistical_verdict 加 final_verdict=DATA_BLOCKED，本轮不能取得影子资格。这是预注册结果而非实现未完成；三项恢复后才重跑 final 层。
...
- 形成日历以独立的 `public.trading_calendar` 为权威来源：读取
  `calendar_date`、`sfe` 与 `deleted_at`，并覆盖到 `data_end` 所在自然月末；
  权威表必须逐自然日完整、日期唯一且有序、`sfe` 为布尔值。完成月定义为
  权威日历中最后交易日不晚于 `data_end` 的月份；这些月份内
  `000905.SH` 的实际交易日集合必须与 `sfe=true` 集合逐日完全一致，
  否则判为 DATA_BLOCKED。当前尚未完成的月份不生成形成日；
...
“输入合同完整”固定指：要求的源表与日期均存在、主键唯一、字段类型合法，并且 size universe 中每只股票都能被归为“合法进入”或一个预登记的排除原因；无法解释的缺失一律先归 DATA_BLOCKED，不能借 COVERAGE_BLOCKED 掩盖 ingestion 问题。
...
- preflight 中设计矩阵结构性秩亏或残差标准差为 0 时判 COVERAGE_BLOCKED；preflight 已通过后再出现同类问题时判实现/数值错误，整次运行失败。
...
然后对每条腿迭代执行 1% 单票封顶，把剩余权重按未封顶的绝对暴露比例重新分配给尚未封顶的股票。若输入合同完整但无法同时满足“权重和为 1、单票不超过 1%、至少 100 只”，preflight 判 COVERAGE_BLOCKED。
...
正式停牌且有交易状态记录时，当日价格按不变处理、权重保留；无法由交易状态解释的缺价属于 DATA_BLOCKED，不在当日把缺价股票权重重新分给其他股票。单只股票出现无法处理的终止缺价时同样判 DATA_BLOCKED。
...
UU、DD、DIV 必须分别在 2014—2017、2018—2020、2021—2023 三个闸门子窗各覆盖至少 10% 的有效交易日；2014—2020 全发现期覆盖率另行报告。频率只检查两个稳定性子窗的系数是否可识别，不进入回归权重、信号权重或候选排序。
...
3. 2×3/5×5 的全格结果已完整生成，没有通过挑格子改变模型。

第 1 项同时承担 2021 年前行业静态近似段与 2021 年后行业 PIT 段的交互方向一致性检查，不再重复设置第二道同义闸门。
...
- load_carry("blend") = 0.5 × IC carry + 0.5 × IM carry；期货尚未上市造成的结构性缺腿按 0；
...
上市后的数据断更不属于“结构性缺腿”，不得按 0 延长回测；按第 5.6 节截到共同有效末日。
...
5. 对候选与基线的配对日收益做 20 日移动块 bootstrap：
   ...
   - 统计量为 Sharpe 差；
   - bootstrap_tail_prob = (1 + count(delta_sharpe <= 0)) / 5001；
6. 对 unified 和 dual-target 两个 tail_prob 做 Holm-style 调整，调整值 < 0.10。

这里的 tail_prob 是 paired moving-block bootstrap 下“Sharpe 差重采样后翻为非正”的经验尾概率，属于置信区间式稳定性度量，不是基于显式可交换零假设的置换 p 值。Holm-style 步骤作为固定两候选的保守多重比较调整，但不宣称在名义 10% 水平获得精确的频率派 FWER 控制。

Holm-style 调整的候选族始终固定为两个。结构闸门失败的候选原始尾概率记为 1，不能通过“先删掉失败候选”缩小调整数量。
...
第 1 层是 run 级阻断，先于任何家族结局：

1. **DATA_BLOCKED**：输入数据合同、法定日滞后敏感性或市值坐标校准使历史裁决无法完成；或者真实首披日、SalG、最新 carry 任一影子前置未就绪。
2. **COVERAGE_BLOCKED**：没有 DATA_BLOCKED，但预飞审计证明固定的 1%/100 只/形成期约束不可行。

若两者同时出现，DATA_BLOCKED 覆盖 COVERAGE_BLOCKED。任一 run 级阻断都使 final_verdict 取对应 BLOCKED；statistical_verdict 若已能合法计算则保留，不能被阻断态抹去。

第 2 层是每个候选独立且互斥的 statistical label：

1. **STOP**：数据与覆盖足以评价，但该候选的核心结构闸门失败。
2. **MEASURE_ONLY**：结构成立，但该候选的生产闸门失败；dual-target 只在 pre-IM 反事实段成立也归此类。
3. **PASS_SHADOW**：该候选通过全部结构、生产、Holm-style 尾概率调整及其适用的 IM 边界闸门。

approximate-PIT 的 STOP/MEASURE_ONLY 必须标为 provisional，不进入“勿重开”清单。候选级三标签不会在同一候选上同时出现，不存在候选内优先级。

第 3 层才聚合家族 statistical_verdict，固定采用 best-wins：

    PASS_SHADOW > MEASURE_ONLY > STOP

即任一候选 PASS_SHADOW，家族 statistical_verdict 就是 PASS_SHADOW；否则任一候选 MEASURE_ONLY，家族为 MEASURE_ONLY；否则为 STOP。反例被明确钉死：{unified=PASS_SHADOW, dual-target=STOP} 聚合为 PASS_SHADOW，unified 可以进入影子候选集。

verdicts.csv 同时保存候选级标签、家族 statistical_verdict 与 run 级 final_verdict。final_verdict 先应用 DATA_BLOCKED/COVERAGE_BLOCKED；仅无 run 级阻断时才等于家族 statistical_verdict。如果两个候选都通过，两者都进入影子期，不用历史指标再选一个。

## 11. 影子期

PASS_SHADOW 后才建立影子运行，不改生产仓位：

进入影子候选集仅表示 statistical_verdict 通过；实际启动影子运行必须满足 final_verdict=PASS_SHADOW。
...
- 新增配对 20 日 moving-block Sharpe 差 bootstrap 与两候选 Holm-style 尾概率调整；
...
| bootstrap.csv | 原始 bootstrap 尾概率、Holm-style 调整值与置信区间 |
...
- 无法解释的缺失触发 DATA_BLOCKED，只有每只股票都有合法进入/排除原因时才允许判 COVERAGE_BLOCKED；
...
- 任一状态在 2014—2017、2018—2020、2021—2023 任一子窗覆盖不足 10% 时，候选失败且不得给稀疏状态加权补救；
...
- 单独 500/1000 指标不增加 Holm-style 调整的候选数；
- 20 日移动块 bootstrap 固定种子可复现；
- 配对 moving-block 检验与 significance.py 的仓位循环移位置换在合成零假设下产生不同、各自正确的结果；
- bootstrap 输出明确为经验尾概率而非置换 p 值；Holm-style 调整使用两个固定候选的原始尾概率，边界 0.10 的通过/失败行为固定；
- MaxDD 恶化 2pp、同窗 long-flat 换手 1.5 倍的边界行为固定；
- 年度集中度字段不参与 PASS/FAIL，修改其值不能改变 verdict；
- run 级阻断固定为 DATA_BLOCKED > COVERAGE_BLOCKED > 任一候选/家族结局；
- 家族 statistical_verdict 固定 best-wins：PASS_SHADOW > MEASURE_ONLY > STOP；
- 聚合断言 {PASS_SHADOW, STOP} → PASS_SHADOW、{MEASURE_ONLY, STOP} → MEASURE_ONLY、{STOP, STOP} → STOP。
...
这里的“五部分设计”指此前逐节确认的：①总体架构；②连续市值×风格暴露与组合；③UU/DD/DIV 市场状态分解；④数据、结构与生产裁决闸门；⑤模块边界、产物、失败处理与测试。

本设计只有在以下条件均满足后才可以进入实施计划：

1. 用户确认本文准确表达了已讨论的五部分设计；
2. 文中不存在未决事项，核心统计口径均已冻结；
