...
1. **size universe**：形成日仍在样本中的 A 股，具备有效总市值和行业标签。m_perp 与目标市值坐标在该集合上计算。
2. **model universe**：size universe 中同时具备合法 style_score 的股票。s_perp、h_perp 及所有风格组合在该集合上计算。
...
1. 对 style_score 和 log(total_market_value) 分别做 5% / 95% 截面缩尾；
2. 对缩尾后的 log 市值取负并标准化，得到 m；m 越大代表越小盘；
3. 行业回归使用截距加 K-1 个行业虚拟变量，截面股票等权；
4. 每个残差暴露最终除以自身截面标准差，均值为 0、方差为 1。

定义：

    m_perp = resid(m | 1, industry)
    s_perp = resid(style_score | 1, industry, m)
    h_raw  = winsorize(s_perp * m_perp, 5%, 95%)
    h_perp = resid(h_raw | 1, industry, s_perp, m_perp)

其中 h_perp 是纯 size × style 交互。使用等权 OLS，不加 Ridge，不按回归拟合优度改变模型。

数值闸门：

- s_perp 与行业、m_perp 的绝对样本相关不超过 1e-8；
- m_perp 与行业的绝对样本相关不超过 1e-8；
- h_perp 与行业、s_perp、m_perp 的绝对样本相关不超过 1e-8；
...
使用同一 model universe 和同一月内持有方法，固定输出：
...
对每个形成日，使用下一形成日至本形成日的非重叠个股收益做截面 OLS：
...
3. 冻结系数的 M1 在 2021—2023 同时满足：
   - 相对 M0 的 OOS R-squared 增量大于 0；
   - 月频 Spearman IC 不低于 M0；
4. 控制 equal_weight 月末信号后的 partial Spearman IC 在 2021—2023 合并样本为正，并在三个自然年中至少两个同号；
5. 上述交互方向和状态增量在 2021 年前行业近似段、2021 年后 PIT 段同方向；
6. 2×3/5×5 的全格结果已完整生成，没有通过挑格子改变模型。

OOS R-squared 的分母固定使用发现期目标收益均值。结构闸门不允许用第二验证窗补救。
...
2. 文中不存在 TBD、TODO 或可由实现者自行选择的核心统计口径；
