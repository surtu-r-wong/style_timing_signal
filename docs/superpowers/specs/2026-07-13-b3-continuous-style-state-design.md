...
先对 unified、dual-target 分别生成候选级 verdict，再聚合家族 verdict：任一候选 PASS_SHADOW 则家族为 PASS_SHADOW；否则有 MEASURE_ONLY 取 MEASURE_ONLY，否则取 STOP。DATA_BLOCKED 与 COVERAGE_BLOCKED 按前述优先级覆盖家族结果。如果两个候选都通过，两者都进入影子期，不用历史指标再选一个。
