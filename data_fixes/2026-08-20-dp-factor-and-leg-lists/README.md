...
- 预期方向：DP 复活会改变全窗复刻序列；0R 锚复现按设计会**有意漂移**（这是
  ...
  条款不变）。
- **2026-08-21 裁决回执**：用户裁 A（整体重登）——锚与地板已更新为修复后新值
  （`backtest/gate0_runner.py` 模块常量），新登记下 0R 三判定全 PASS；裁决记录
  与新旧对照表 = `docs/plans/2026-08-20-data-foundation-repair.md` §7。本目录
  首跑备份即旧锚时代的永久存证，不动。
